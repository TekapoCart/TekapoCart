...
if (!defined('_SMILEPAY_CREDITSTATUS_')) {
	...
	WHERE id_lang = \''.pSQL('1').'\' AND  template = \'SmilePay_credit_status\'');

	if ($rq && isset($rq['id_order_state']) && intval($rq['id_order_state']) > 0) {

		// order status exists in the table - define it.

		define('_SMILEPAY_CREDITSTATUS_', $rq['id_order_state']);
        ...
        $sp_modulename = 'smilepay_credit';
        $sp_template = 'SmilePay_credit_status';
        ...
        if($sp_insert)
        {
		    ...
		    Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'order_state_lang` (`id_order_state`, `id_lang`, `name`, `template`)VALUES(' . intval($stateid) . ', 1,\''.iconv("big5","UTF-8",'信用卡付款中').'\', \'SmilePay_credit_status\')');

		    Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'order_state_lang` (`id_order_state`, `id_lang`, `name`, `template`)VALUES(' . intval($stateid) . ', 2,\''.iconv("big5","UTF-8",'信用卡付款中').'\', \'SmilePay_credit_status\')');
