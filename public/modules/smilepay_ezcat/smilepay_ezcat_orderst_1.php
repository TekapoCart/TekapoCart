...
if (!defined('_SMILEPAY_EZCATSTATUS_')) {
	...
	if ($rq && isset($rq['id_order_state']) && intval($rq['id_order_state']) > 0) {
        ...
        if($sp_insert)
        {
		    ...
		    Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'order_state_lang` (`id_order_state`, `id_lang`, `name`, `template`)VALUES(' . intval($stateid) . ', 1,\''.iconv("big5","UTF-8",'黑貓取貨付款處理中').'\', \'SmilePay_ezcat_status\')');
		    Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'order_state_lang` (`id_order_state`, `id_lang`, `name`, `template`)VALUES(' . intval($stateid) . ', 2,\''.iconv("big5","UTF-8",'黑貓取貨付款處理中').'\', \'SmilePay_ezcat_status\')');
		    copy((dirname(__file__) .  "/icons/smilepay.gif"),(dirname(dirname(dirname(__file__)))."/img/os/".intval($stateid).".gif"));
        }
		define('_SMILEPAY_EZCATSTATUS_', $stateid);
	}
} 
