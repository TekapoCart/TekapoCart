...
class LoyaltyModule extends ObjectModel
{
        ...
        if (Validate::isLoadedObject($cart)) {
            ...
            if (isset($newProduct) AND !empty($newProduct)) {
                ...
                if ($taxesEnabled == PS_TAX_EXC) {
                    ...
                    $cartProductsNew['price_wt'] = number_format($newProduct->getPrice(true, (int)$newProduct->getIdProductAttributeMostExpensive()),
                        2, '.', '');
                }
    ...
    public static function getAllByIdCustomer($id_customer, $id_lang, $onlyValidate = false, $pagination = false, $nb = 10, $page = 1)
    {
        ...
        $query = '
		SELECT f.id_order AS id, f.date_add AS date, (o.total_paid - o.total_shipping) total_without_shipping, o.reference AS ref, f.points, f.id_loyalty, f.id_loyalty_state, fsl.name state
		FROM `' . _DB_PREFIX_ . 'loyalty` f
