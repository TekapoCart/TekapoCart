...
class LoyaltyDefaultModuleFrontController extends ModuleFrontController
{
    ...
    public function __construct()
    {
        ...
        smartyRegisterFunction($this->context->smarty, 'function', 'summarypaginationlink',
            array('LoyaltyDefaultModuleFrontController', 'getSummaryPaginationLink'));
    }
    ...
    public function postProcess()
    {
        ...
        if (empty(Tools::getValue('process'))) {
            $this->assignSummaryExecution();
        }
    }
    ...
    {
        $customer_points = (int)LoyaltyModule::getPointsByCustomer($this->context->customer->id);

        $orders = LoyaltyModule::getAllByIdCustomer($this->context->cookie->id_customer, $this->context->cookie->id_lang, true, false);
        $ref_list = [];
        ...
        $customerCountry = $customer->getCurrentCountry((int)$this->context->customer->id);
        ...
        if ($customer_points > 0) {
            ...
            $cart_rule->description = implode(",", $ref_list);
            ...
            // 幾日內完成兌換
            $valid_days = (int)Configuration::get('PS_LOYALTY_VALIDITY_PERIOD');
            if ($valid_days > 0) {
                $cart_rule->date_to = date('Y-m-d H:i:s', strtotime($cart_rule->date_from . " +$valid_days days"));
            }
    ...
    public function initContent()
    {
        ...
        $this->context->smarty->tpl_vars['page']->value['body_classes']['page-loyalty-account'] = true;
    ...
    public function assignSummaryExecution()
    {
        ...
        $displayorders = LoyaltyModule::getAllByIdCustomer(
            ...
            ((int)Tools::getValue('n') > 0 ? (int)Tools::getValue('n') : 100),
        ...
        $this->context->smarty->assign(
            array(
                ...
                'validity_period' => (float)Configuration::get('PS_LOYALTY_VALIDITY_PERIOD'),
