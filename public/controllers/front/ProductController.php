...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    public function canonicalRedirection($canonical_url = '')
    {
        if (Validate::isLoadedObject($this->product)) {
            if (!$this->product->hasCombinations()) {
                ...
                // suzy: 2018-12-31 抓取至少庫存 1 的屬性（多屬性若無庫存，自動找到有庫存的規格）
                // $_GET['id_product_attribute'] = Product::getDefaultAttribute($this->product->id);
                $_GET['id_product_attribute'] = Product::getDefaultAttribute($this->product->id, 1);
            } else {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            foreach ($attributes_groups as $k => $row) {
                ...
                $groups[$row['id_attribute_group']]['attributes'][$row['id_attribute']] = array(
                    ...
                    // suzy: 2018-11-12 讓屬性圖片 cdn
                    // 'texture' => (@filemtime(_PS_COL_IMG_DIR_ . $row['id_attribute'] . '.jpg')) ? _THEME_COL_DIR_ . $row['id_attribute'] . '.jpg' : '',
                    'texture' => (@filemtime(_PS_COL_IMG_DIR_ . $row['id_attribute'] . '.jpg')) ? 'https://' . Tools::getMediaServer(_THEME_COL_DIR_ . $row['id_attribute'] . '.jpg') . _THEME_COL_DIR_ . $row['id_attribute'] . '.jpg' : '',
    ...
    private function getIdProductAttributeByRequest()
    {
        ...
        if (!$requestedIdProductAttribute) {
            $requestedIdProductAttribute = Product::getDefaultAttribute($this->product->id, 1);
        }
