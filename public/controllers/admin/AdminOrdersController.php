                ...
                if (Tools::isSubmit('partialRefundProduct') && ($refunds = Tools::getValue('partialRefundProduct')) && is_array($refunds)) {
                    ...
                    if ($amount >= 0) {
                        if (!OrderSlip::create(
                            ...
                            $params['{firstname}'] = Tools::maskString($customer->firstname, 'name');
    ...
    public function renderView()
    {
        ...
        foreach ($states as $state) {
            ...
            if (in_array($state['id_order_state'], $unpaid_state_ids)) {
                ...
                $sorted_states['shipment'][$state['id_order_state']] = '﹂' . $state['name'];
            } else if (in_array($state['id_order_state'], $error_state_ids)) {
                $sorted_states['error'][$state['id_order_state']] = '﹂' . $state['name'];
            } else if (in_array($state['id_order_state'], $archived_state_ids)) {
                $sorted_states['archived'][$state['id_order_state']] = '﹂' . $state['name'];
            }
        }
