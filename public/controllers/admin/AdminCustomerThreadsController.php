...
//            ),
    ...
    public function renderList()
    {
        ...
        // suzy: 2018-09-17 調換姓名順序、完整顯示員工姓名
//        $this->_select = '
...
//			) as employee';
        ...
        $this->_select = '
			CONCAT(c.`lastname`," ",c.`firstname`) as customer, cl.`name` as contact, l.`name` as language, group_concat(cm.`message`) as messages, cm.private,
			(
				SELECT IFNULL(CONCAT(e.`lastname`," ",e.`firstname`), "--")
				FROM `' . _DB_PREFIX_ . 'customer_message` cm2
				INNER JOIN ' . _DB_PREFIX_ . 'employee e
					ON e.`id_employee` = cm2.`id_employee`
				WHERE cm2.id_employee > 0
