        ...
        if ($this->access('edit')) {
            ...
            if (Validate::isLoadedObject($module)) {
                if (!$module->getPermission('configure')) {
                    ...
                    // suzy: 2018-08-01 從金物流來的回金物流去
                    if (in_array(Tools::getValue('module_name'), ['ps_cashondelivery', 'ps_wirepayment', 'paypal'])
                        || strstr(Tools::getValue('module_name'), 'smilepay')
                    ) {
                        Tools::redirectAdmin($this->context->link->getAdminLink('AdminPayment', true));
                        exit;
                    }
