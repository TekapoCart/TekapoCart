...
class AdminModulesPositionsControllerCore extends AdminController
{
    ...
    public $controller_name_mapping = [
        'address' => '會員中心 配送資訊',
        'addresses' => '會員中心 新增/修改配送資訊',
        'attachment',
        'auth' => '會員登入',
        'bestsales' => '熱賣商品',
        'cart' => '購物車',
        ...
        'newproducts' => '新品上架',
        'order' => '訂單明細',
        'orderconfirmation' => '訂單完成',
        'orderdetail' => '會員中心 訂單明細',
        'orderfollow',
        'orderreturn',
        'orderslip',
        'pagenotfound',
    ...
    public $module_name_mapping = [
        'module-loyalty-default' => '會員中心 點數回饋',
    ...
    public function initMain()
    {
        ...
        // suzy: 2018-08-13 加入 order_by sort
        $hooks = Hook::getHooks(false, false, 'sort');
        ...
        foreach ($hooks as $key => $hook) {
            ...
            if (_PS_MODE_DEV_ === false) {
                if (!in_array($hook['name'], $this->custom_display_hooks)) {
                    unset($hooks[$key]);
                }
            }
...
//        $content .= '<option disabled="disabled">' . $this->trans('____________ CORE ____________', array(), 'Admin.Design.Feature') . '</option>';
        ...
        foreach ($controllers as $k => $v) {
            // suzy: 2018-08-13 頁面名稱 顯示中文方便閱讀
            $name = array_key_exists($k, $this->controller_name_mapping) ? $this->controller_name_mapping[$k] : '';
            if ($name === FALSE) {
                continue;
            }
            // suzy: 2018-08-13 頁面名稱 顯示中文方便閱讀
...
//            $content .= '<option disabled="disabled">____________ ' . $label . ' ____________</option>';
    ...
    public function ajaxProcessGetPossibleHookingListForModule()
    {
        ...
        if (_PS_MODE_DEV_ === false) {
            ...
            $possibleHooksList = $module_instance->getPossibleHooksList();
            $hooks = [];
            foreach ($possibleHooksList as $key => $hook) {
                if (in_array($hook['name'], $this->custom_display_hooks)) {
                    unset($hooks[$key]);
                    $hooks[] = $hook;
                }
