...
class AdminThemesControllerCore extends AdminController
{
    ...
    public function initPageHeaderToolbar()
    {
        ...
        // suzy: 2018-08-21 dev mode 再顯示 匯出、匯入
        if (_PS_MODE_DEV_) {
    ...
    public function renderOptions()
    {
                ...
                'fields' => array(
                    ...
                    */
                    'PS_FAVICON' => array(
                        ...
                        */
                        'title' => '瀏覽器<br>32x32px',
                        'hint' => Configuration::get('PS_FAVICON'),
                        'type' => 'file',
                        'name' => 'PS_FAVICON',
                        'tab' => 'icons',
                        'thumb' => _PS_IMG_ . Configuration::get('PS_FAVICON') . (Tools::getValue('conf') ? sprintf('?%04d', rand(0, 9999)) : ''),
                    ),
                    // suzy: 2018-10-19 新增 favicon 欄位
                    'PS_FAVICON_IPHONE' => array(
                        'title' => 'iPhone<br>120x120px',
                        'hint' => Configuration::get('PS_FAVICON_IPHONE'),
                        'type' => 'file',
                        'name' => 'PS_FAVICON_IPHONE',
                        'tab' => 'icons',
                        'thumb' => _PS_IMG_.Configuration::get('PS_FAVICON_IPHONE').(Tools::getValue('conf') ? sprintf('?%04d', rand(0, 9999)) : '')
                    ),
                    'PS_FAVICON_IPAD' => array(
                        'title' => 'iPad, iPad Mini<br>152x152px',
                        'hint' => '檔名：' . Configuration::get('PS_FAVICON_IPAD'),
                        'type' => 'file',
                        'name' => 'PS_FAVICON_IPAD',
                        'tab' => 'icons',
                        'thumb' => _PS_IMG_.Configuration::get('PS_FAVICON_IPAD').(Tools::getValue('conf') ? sprintf('?%04d', rand(0, 9999)) : '')
                    ),
                    'PS_FAVICON_IPAD_RETINA' => array(
                        'title' => 'iPad Pro (Retina)<br>167x167px',
                        'hint' => '檔名：' . Configuration::get('PS_FAVICON_IPAD_RETINA'),
                        'type' => 'file',
                        'name' => 'PS_FAVICON_IPAD_RETINA',
                        'tab' => 'icons',
                        'thumb' => _PS_IMG_.Configuration::get('PS_FAVICON_IPAD_RETINA').(Tools::getValue('conf') ? sprintf('?%04d', rand(0, 9999)) : '')
                    ),
                    'PS_FAVICON_IPHONE_RETINA' => array(
                        'title' => 'iPhone (Retina)<br>180x180px',
                        'hint' => '檔名：' . Configuration::get('PS_FAVICON_IPHONE_RETINA'),
                        'type' => 'file',
                        'name' => 'PS_FAVICON_IPHONE_RETINA',
                        'tab' => 'icons',
                        'thumb' => _PS_IMG_.Configuration::get('PS_FAVICON_IPHONE_RETINA').(Tools::getValue('conf') ? sprintf('?%04d', rand(0, 9999)) : '')
                    ),
                    'PS_FAVICON_ANDROID' => array(
                        'title' => 'Android<br>192x192px',
                        'hint' => '檔名：' . Configuration::get('PS_FAVICON_ANDROID'),
                        'type' => 'file',
                        'name' => 'PS_FAVICON_ANDROID',
                        'tab' => 'icons',
                        'thumb' => _PS_IMG_.Configuration::get('PS_FAVICON_ANDROID').(Tools::getValue('conf') ? sprintf('?%04d', rand(0, 9999)) : '')
                    ),
                    'PS_FAVICON_MICROSOFT' => array(
                        'title' => 'Windows 10 / Edge<br>310x310px',
                        'hint' => '檔名：' . Configuration::get('PS_FAVICON_MICROSOFT'),
                        'type' => 'file',
                        'name' => 'PS_FAVICON_MICROSOFT',
                        'tab' => 'icons',
                        'thumb' => _PS_IMG_.Configuration::get('PS_FAVICON_MICROSOFT').(Tools::getValue('conf') ? sprintf('?%04d', rand(0, 9999)) : '')
                    ),
                ),
                'after_tabs' => array(
        ...
        // suzy: 2019-01-21 隱藏 官網、銀戒 佈景
        foreach ($other_themes as $key => $theme) {
            if (in_array($key, ['simplicity_silverchieh', 'simplicity_tekapo'])) {
                if (!in_array($this->context->shop->domain, ['shop1.presta.shop', 'www.tekapo.io', 'silver.tekapo.space'])) {
                    unset($other_themes[$key]);
                }
            }
        }
    ...
    public function initConfigureLayouts()
    {
        ...
        // suzy: 2018-08-20 排掉不需要的頁面
        $pages = Meta::getAllMeta($this->context->language->id);
        foreach ($pages as $key => $page) {
            if (in_array($page['id_meta'], [51])) {
                continue;
            }
            if ($key >= 28) {
                unset($pages[$key]);
    ...
    protected function postProcessSubmitOptionsConfiguration()
    {
        try {
            ...
            if (Tools::getValue('PS_FAVICON_IPHONE')) {
                $this->logo_uploader->updateFavicon('PS_FAVICON_IPHONE', 'touch-icon-iphone');
                $this->redirect_after = self::$currentIndex . '&token=' . $this->token;
            }
            if (Tools::getValue('PS_FAVICON_IPAD')) {
                $this->logo_uploader->updateFavicon('PS_FAVICON_IPAD', 'touch-icon-ipad');
                $this->redirect_after = self::$currentIndex . '&token=' . $this->token;
            }
            if (Tools::getValue('PS_FAVICON_IPAD_RETINA')) {
                $this->logo_uploader->updateFavicon('PS_FAVICON_IPAD_RETINA', 'touch-icon-ipad-retina');
                $this->redirect_after = self::$currentIndex . '&token=' . $this->token;
            }
            if (Tools::getValue('PS_FAVICON_IPHONE_RETINA')) {
                $this->logo_uploader->updateFavicon('PS_FAVICON_IPHONE_RETINA', 'touch-icon-iphone-retina');
                $this->redirect_after = self::$currentIndex . '&token=' . $this->token;
            }
            if (Tools::getValue('PS_FAVICON_ANDROID')) {
                $this->logo_uploader->updateFavicon('PS_FAVICON_ANDROID', 'android-icon');
                $this->redirect_after = self::$currentIndex . '&token=' . $this->token;
            }
            if (Tools::getValue('PS_FAVICON_MICROSOFT')) {
                $this->logo_uploader->updateFavicon('PS_FAVICON_MICROSOFT', 'microsoft-icon');
                $this->redirect_after = self::$currentIndex . '&token=' . $this->token;
            }

            // suzy: 2018-09-12 清 cache 不然前台會破圖
            Tools::clearAllCache();
