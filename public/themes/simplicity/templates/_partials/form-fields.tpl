    ...
    <div class="col-md-8{if ($field.type === 'radio-buttons')} form-control-valign{/if}">
      ...
      {if $field.type === 'select'}
        ...
        {block name='form_field_item_select'}
          <select class="form-control form-control-select" name="{$field.name}" {if $field.required}required{/if}>
            <option value disabled selected>{l s='-- please choose --' d='Shop.Forms.Labels'}「{$field.label}」</option>
      ...
      {elseif $field.type === 'countrySelect'}
        ...
        {block name='form_field_item_country'}
          ...
          >
            <option value disabled selected>{l s='-- please choose country --' d='Shop.Forms.Labels'}</option>
            {foreach from=$field.availableValues item="label" key="value"}
              <option value="{$value}" {if $value eq $field.value} selected {/if}>{$label}</option>
            {/foreach}
          </select>
