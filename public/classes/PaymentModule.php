...
abstract class PaymentModuleCore extends Module
{
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    $extra_info_html = '';
                    $extra_info_html .= $this->getEmailTemplateContent('order_conf_' . $order->module . '.tpl', Mail::TYPE_HTML, $extra_info_data);
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        '{delivery_address1}' => Tools::maskString($delivery->address1, 'address'),
                        '{delivery_address2}' => $delivery->address2,
                        '{delivery_city}' => $delivery->city,
                        '{delivery_postal_code}' => $delivery->postcode,
                        '{delivery_country}' => $delivery->country,
                        '{delivery_state}' => $delivery->id_state ? $delivery_state->name : '',
                        '{delivery_phone}' => ($delivery->phone) ? $delivery->phone : Tools::maskString($delivery->phone_mobile, 'phone'),
                        ...
                        '{invoice_address1}' => Tools::maskString($invoice->address1, 'address'),
                        '{invoice_city}' => $invoice->city,
                        '{invoice_postal_code}' => $invoice->postcode,
                        '{invoice_country}' => $invoice->country,
                        '{invoice_state}' => $invoice->id_state ? $invoice_state->name : '',
                        '{invoice_phone}' => ($invoice->phone) ? $invoice->phone : Tools::maskString($invoice->phone_mobile, 'phone'),
    ...
    {
        // suzy: 2018-09-13 加遮罩
        return AddressFormat::generateAddressWithMask($the_address, array('avoid' => array()), $line_sep, ' ', $fields_style);
