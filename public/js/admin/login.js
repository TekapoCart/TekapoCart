$(document).ready(function() {
	...
	//Preload images
	$('<img/>')[0].src = img_dir+'teacher9-login.png'; // suzy: 2018-10-29 後台登入 LOGO
    $('<img/>')[0].src = img_dir+'teacher9-login-flip.png';

	$('button[name="submitLogin"]').on('mouseover', function() {
		$('#shop-img img').attr('src', img_dir+'teacher9-login-flip.png'); // suzy: 2018-10-29 後台登入 LOGO
	});

	$('button[name="submitLogin"]').on('mouseout', function() {
		$('#shop-img img').attr('src', img_dir+'teacher9-login.png'); // suzy: 2018-10-29 後台登入 LOGO
	});
