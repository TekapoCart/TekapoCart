              ...
              {if $k != 'back' && $k != 'modules-list'}
                ...
                <li style="padding-right: 15px !important;">
                  <a id="page-header-desc-{$table}-{if isset($btn.imgclass)}{$btn.imgclass|escape}{else}{$k}{/if}"
                    ...
                    {* suzy: 2018-09-17 加 i 跟 span(原 div) 的 style *}
                    <i
                      class="{if isset($btn.icon)}{$btn.icon|escape}{else}process-icon-{if isset($btn.imgclass)}{$btn.imgclass|escape}{else}{$k}{/if}{/if}{if isset($btn.class)} {$btn.class|escape}{/if}" style="display: inline-block;"></i>
                    {* suzy: 2018-09-17 div 改 span *}
                    <span{if isset($btn.force_desc) && $btn.force_desc == true } class="locked"{/if} style="vertical-align: super; font-size: 14px;">{$btn.desc|escape}</span>
                  </a>
                </li>
              {/if}
            {/foreach}
            {* suzy: 2018-06-12 隱藏 推薦模塊和服務
