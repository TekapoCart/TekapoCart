...
<div class="row">
	<div class="col-lg-9">
		<div class="panel">
			{* suzy: 2018-08-13 隱藏下拉搜尋 <form class="well form-horizontal" id="position_filer">
				<div class="row">
				...
				<form method="post" action="{$url_submit|escape:'html':'UTF-8'}" >
					...
					<section class="hook_panel {if $hook['position'] == 0}hook_position{/if}" {if $hook['position'] == 0}style="display:none;"{/if}>
						...
						<header class="hook_panel_header">
                            {* suzy: 2018-08-13 開放顯示 title *}
							<span class="hook_name">{$hook['sort']}. {$hook['title']} [{$hook['name']}]</span>
							<!-- <span class="hook_title">{$hook['title']}</span> -->
							{* suzy: 2018-08-13 隱藏右側 checkbox <span class="badge pull-right">
						...
						<ul class="list-unstyled{if $hook['modules']|count > 1} sortable{/if}">
							...
							<li id="{$hook['id_hook']|intval}_{$module['instance']->id|intval}" class="module_position_{$module['instance']->id|intval} module_list_item{if $can_move && $hook['module_count'] >= 2} draggable{/if}">
								{* suzy: 2018-08-13 隱藏 checkbox <div class="module_col_select">
...
{/foreach}
					{* suzy: 2018-08-13 隱藏 「解除選取」按鈕<div id="unhook_button_position_bottom">
	...
	{* suzy: 2018-08-13 隱藏右區塊 「已選 1 個模組 已選模組」 <div class="col-lg-3">
