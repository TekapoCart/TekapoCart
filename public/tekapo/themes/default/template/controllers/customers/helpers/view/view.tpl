		...
		<div class="col-lg-6">
			<div class="panel clearfix">
				...
				<div class="form-horizontal">
                    ...
                    {* suzy: 2018-07-09 顯示 FB 頭像 *}
                    {if $fb_social_id > 0}
						<div style="position: absolute; left: 25px; top: 50px">
							<img src='https://graph.facebook.com/{$fb_social_id}/picture?type=square'/>
						</div>
                    {/if}
				...
				{if $customer->isGuest()}
                    {* suzy: 2018-06-22 加了 p tag *}
					<p><br>{l s='This customer is registered as a Guest.' d='Admin.Orderscustomers.Feature'}</p>
					{if !$customer_exists}
			...
			<div class="panel">
				...
				{if count($addresses)}
					<table class="table">
						<thead>
							<tr>
								{* suzy: 2018-09-26 隱藏 公司<th><span class="title_box ">{l s='Company' d='Admin.Global'}</span></th>*}
								<th><span class="title_box ">{l s='Name' d='Admin.Global'}</span></th>
						...
						<tbody>
							...
							<tr>
								{* suzy: 2018-09-26 隱藏 公司<td>{if $address['company']}{$address['company']}{else}--{/if}</td>*}
								<td>{* suzy: 2018-09-17 調整姓氏 *}{$address['lastname']} {$address['firstname']}</td>
								<td>{* suzy: 2018-09-26 調整地址 *}{if $address['address1'] !== 'N/A'}{$address['address1']}{/if} {if $address['address2']}{$address['address2']}{/if} {if $address['postcode'] !== '00'}{$address['postcode']}{/if} {$address['city']}</td>
								<td>{$address['country']}</td>
								<td>{* suzy: 2018-09-26 調整電話 *}
                                    {if $address['phone_mobile']}{$address['phone_mobile']}{/if}
                                    {if $address['phone']}<br />{$address['phone']}{/if}
									{*if $address['phone']}
										{$address['phone']}
