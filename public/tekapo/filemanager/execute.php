            ...
            if ($delete_files) {
                ...
                if (file_exists($path_thumb)) {
                    ...
                    if (file_exists(_GCP_KEY_) && strlen(_GCP_BUCKET_) > 0) {
                        $storage = new StorageClient(['keyFilePath' => _GCP_KEY_]);
                        $bucket = $storage->bucket(_GCP_BUCKET_);
                        $object = $bucket->object(str_replace(_PS_ROOT_DIR_ . '/', '', $path_thumb));
                        if ($object->exists()) {
                            $object->delete();
                        }
                    }
                    ...
                    foreach ($relative_path_from_current_pos as $k => $path) {
                        ...
                        if (file_exists($info['dirname'].'/'.$path.$relative_image_creation_name_to_prepend[$k].$info['filename'].$relative_image_creation_name_to_append[$k].'.'.$info['extension'])) {
                            ...
                            if (file_exists(_GCP_KEY_) && strlen(_GCP_BUCKET_) > 0) {
                                $storage = new StorageClient(['keyFilePath' => _GCP_KEY_]);
                                $bucket = $storage->bucket(_GCP_BUCKET_);
                                $object = $bucket->object(str_replace(_PS_ROOT_DIR_ . '/', '', $info['dirname'].'/'.$path.$relative_image_creation_name_to_prepend[$k].$info['filename'].$relative_image_creation_name_to_append[$k].'.'.$info['extension']));
                                if ($object->exists()) {
                                    $object->delete();
                                }
                            }
                    ...
                    foreach ($fixed_path_from_filemanager as $k => $path) {
                        ...
                        if (file_exists($base_dir.$fixed_image_creation_name_to_prepend[$k].$info['filename'].$fixed_image_creation_to_append[$k].'.'.$info['extension'])) {
                            ...
                            if (file_exists(_GCP_KEY_) && strlen(_GCP_BUCKET_) > 0) {
                                $storage = new StorageClient(['keyFilePath' => _GCP_KEY_]);
                                $bucket = $storage->bucket(_GCP_BUCKET_);
                                $object = $bucket->object(str_replace(_PS_ROOT_DIR_ . '/', '', $base_dir.$fixed_image_creation_name_to_prepend[$k].$info['filename'].$fixed_image_creation_to_append[$k].'.'.$info['extension']));
                                if ($object->exists()) {
                                    $object->delete();
                                }
                            }
